...
if(SD){uint8_t anomalySD[ysize][xsize];memset(anomalySD, 0, sizeof(anomalySD[0][0]) * ysize * xsize);}
...
if(SD){uint8_t sdOUT[ysize][xsize];memset(sdOUT, 0, sizeof(sdOUT[0][0]) * ysize * xsize);}
...
for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {
  if(currVF[y][x] != 255){//valid land observation
    ...
    if(count>=4 or (currVF[y][x]>90 and count>=1 and !SD)){//min baseline count of 4 or currVF>90
      ...
      if(SD){
        ...
        else{anomalySD[y][x] = 0;}
        sdOUT[y][x]=sd;
      }
    }else{
      minOUT[y][x]=200; countOUT[y][x] = 0; 
      if(currVF[y][x]>90){anomaly[y][x]=0;}//set anomaly to zero if currVF is >90 and no baseline
      else{anomaly[y][x]=255;}
      if(SD){sdOUT[y][x]=200; anomalySD[y][x]=255;}
    }
  }else{
    minOUT[y][x]=255;anomaly[y][x]=255;
    if(SD){anomalySD[y][x]=255;sdOUT[y][x]=255;}
  }
}}
...
if(SD){papszMetadataSD = CSLDuplicate(papszMetadata);}
