...
def makeBrowse(OUT_ID,outdir):
  try:
    ...
    if os.path.exists(pngFile):
      subprocess.run(["mv "+pngFile +" " + outdir+"/"+OUT_ID+"_VEG-DIST-STATUS_org.png"],capture_output=True,shell=True)
    ...
    if os.path.exists(hiresFile):
      subprocess.run(["mv "+hiresFile +" " + outdir+"/"+OUT_ID+"_VEG-DIST-STATUS_browse_org.tif"],capture_output=True,shell=True)
    if not os.path.exists(hiresFile):
      ...
      #print("gdalbuildvrt "+outdir+"/temp.vrt "+inputfile)
