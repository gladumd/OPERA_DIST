...
dateStart = '2023-07-01T05:00:00Z'
dateEnd = '2023-10-01T05:00:00Z'

dataStartRelease = "2023-02-24T05:00:00Z"
...
strCommand = "SELECT HLS_ID, DIST_ID, statusFlag, availableTime, downloadTime,processedTime, sensingTime,Errors FROM fulltable WHERE downloadTime >= ? AND downloadTime <= ? AND sensingTime>=?"
cursor.execute(strCommand,(dateStart,dateEnd,dataStartRelease,))
rows = cursor.fetchall()

strCommandStatus = "SELECT COUNT(*),statusFlag FROM fulltable WHERE downloadTime >= ? AND downloadTime <= ? AND sensingTime>=? group by statusFlag"
...
outname_report = indir + "ProductStatus_"+dateStart[0:10]+"_"+dateEnd[0:10]+".csv"
...
outname_report_datacount = indir + "OPERA_UMD_SDS_Activity_Report_"+dateStart[0:10]+"_"+dateEnd[0:10]+".csv"
