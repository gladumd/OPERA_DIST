...
subprocess.run(["sqlite3 -header -csv /gpfs/glad3/HLSDIST/System/database/database.db.bak \"select * from fulltable WHERE sensingTime>\'2024001T000000\';\" > export.csv"],capture_output=True,shell=True)
with open("export.csv",'r') as NEW:
  lines = NEW.read().splitlines()[1:]

subprocess.run(["sqlite3 -header -csv database.db \"select * from fulltable WHERE sensingTime>\'2024001T000000\'\" > exists.csv"],capture_output=True,shell=True)
