...
def send(reportDate):
  ...
  if os.path.exists("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/sentToLP_v1ANN_"+reportDate+".rpt"):
    with open(reconciliationFile,'w') as noti:
      noti.write("{\"report\": {\"uri\": \"https://glad.umd.edu/projects/opera/ingestReports/sentToLP_v1ANN_"+reportDate+".rpt\"}}\n")
...
def receive(reportDate):
  if os.path.exists("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/sentToLP_v1ANN_" +reportDate+".rpt"):
    response = subprocess.run(["module load awscli;source /gpfs/glad3/HLSDIST/System/user.profile; aws s3 cp s3://lp-prod-reconciliation/reports/sentToLP_v1ANN_"+reportDate+".json /gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/report_v1ANN_"+reportDate+".json"],capture_output=True,shell=True)
    ...
    if response.stderr.decode().strip() != "":
      ...
      errors = extractErrors("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/report_v1ANN_"+reportDate+".json",reportDate)

def extractErrors(reportfile,reportDate):
  with open(reportfile) as file:
    line = file.read()
    results = json.loads(line)
  counts = results[0]["OPERA_L3_DIST-ANN-HLS_V1___1"]
  with open("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/ANNSTATS_v1.csv",'a') as DAILY:
  ...
  if int(counts["failed"])+int(counts["missing"])+int(counts["other"])+int(counts["cksum_err"]) > 0:
    ...
    for flag in errors.keys():
      if len(errors[flag]) > 0:
        with open("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/"+reportDate+"_v1ANN_"+flag+".txt", 'w') as out:
          for g in errors[flag]:
            out.write(g+"\n")
        with open("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/"+reportDate+"_v1ANN_"+flag+"_tile.txt", 'w') as out:
          for g in errors[flag]:
            (OPERA,L3,DIST,tile,year,productionTime,resolution,version) = g.split('_')
            out.write(tile[1:]+"\n")
    #resend(errors)
    return errors
  return "NA"
...
if __name__=='__main__':
  ...
  if sys.argv[1] == "SEND":
    ...
    #reportDate = "20230212"
    extractErrors("/gpfs/glad3/HLSDIST/LP-DAAC/ingestReports/report_v1ANN_"+reportDate+".json",reportDate)
  else:
