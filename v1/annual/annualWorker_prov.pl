...
for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++){
    if(vegconf[y][x] > outvegconf[y][x] and vegstatus[y][x]!=FIRSTHI and vegstatus[y][x]!=FIRSTLO){
      index[y][x]==i;
    ...
    if((vegstatus[y][x]>0 or vegstatus[y][x]<9) and !inevent[y][x]){
      inevent[y][x]=true;
      if(countevents[y][x]==255){countevents[y][x]=1;}
      else if(countevents[y][x]<254){countevents[y][x]++;}
    }else if(inevent[y][x] and (vegstatus[y][x]==0 or vegstatus[y][x]==9 or vegstatus[y][x]==10)){
      inevent[y][x]=false;
      ongoing[y][x] = false;
    }
...
for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {
  if(outvegstatus[y][x]==0){//outvegconf[y][x]<confidenceThreshold and outvegconf[y][x]>=0){//formerly == 0
    outveghist[y][x] = 200;
    outveganommax[y][x] = 0;
    outvegcount[y][x] = 0;
...
if(datesNeeded[i]){
  ...
  for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {
    if(outvegstatus[y][x]>0 and index[y][x]==i){// outvegconf[y][x] >= confidenceThreshold and index[y][x]==i){";
...
currMetadata = CSLSetNameValue( currMetadata, \"flag_values\", \"0,2,3,5,6,7,8,9,10,255\");
currMetadata = CSLSetNameValue( currMetadata, \"flag_meanings\", \"no_disturbance,prov_<50%,confirmed_<50%_ongoing,prov_>=50%,confirmed_>=50%_ongoing,confirmed_<50%_completed,confirmed_>=50%_completed,confirmed_<50%_previous_year,confirmed_>=50%_previous_year,no_data\");
...
for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++){
    if(genconf[y][x] > outgenconf[y][x] and genstatus[y][x]!=FIRSTHI and genstatus[y][x]!=FIRSTLO){
      index[y][x]==i;
      outgenconf[y][x] = genconf[y][x];
      outgendate[y][x] = gendate[y][x];
      if(gendate[y][x]<$startdoy){
        if(genstatus[y][x]==3){outgenstatus[y][x] = 9;prevyear[y][x]=1;}////////////////////////what about prov
        else if(genstatus[y][x]==6){outgenstatus[y][x] = 10;prevyear[y][x]=2;}////////////////////////what about prov
      } else {
    ...
    if((genstatus[y][x]>0 or genstatus[y][x]<9) and !inevent[y][x]){
      inevent[y][x]=true;
      if(countevents[y][x]==255){countevents[y][x]=1;}
      else if(countevents[y][x]<254){countevents[y][x]++;}
    }else if(inevent[y][x] and (genstatus[y][x]==0 or genstatus[y][x]==9 or genstatus[y][x]==10)){
      inevent[y][x]=false;
      ongoing[y][x] = false;
    }
...
print OUT"
for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {
  if(outgenstatus[y][x]==0){//outgenconf[y][x]<confidenceThreshold and outgenconf[y][x]>=0){//formerly == 0
    outgenanommax[y][x] = 0;
    outgencount[y][x] = 0;
...
if(datesNeeded[i]){
  ...
  for(y=0; y<ysize; y++) {for(x=0; x<xsize; x++) {
    if(outgenstatus[y][x]>0 and index[y][x]==i){//outgenconf[y][x] >= confidenceThreshold and index[y][x]==i){";
...
currMetadata = CSLDuplicate(papszMetadata);
currMetadata = CSLSetNameValue( currMetadata, \"flag_values\", \"0,2,3,5,6,7,8,9,10,255\");
currMetadata = CSLSetNameValue( currMetadata, \"flag_meanings\", \"no_disturbance,prov_low,confirmed_low_ongoing,prov_high,confirmed_high_ongoing,confirmed_low_completed,confirmed_high_completed,confirmed_low_previous_year,confirmed_high_previous_year,no_data\");
currMetadata = CSLSetNameValue( currMetadata, \"Units\", \"unitless\");
OUTGDAL->SetGeoTransform(GeoTransform); OUTGDAL->SetProjection(OUTPRJ); OUTBAND = OUTGDAL->GetRasterBand(1);
